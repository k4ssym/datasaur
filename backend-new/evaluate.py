...
async def run_evaluation(
    ...
    """Run evaluation on JSON files in the dataset directory. Use limit for quick test."""
    console = Console()
    ...
    if limit is not None and limit > 0:
        ...
        console.print(f"[yellow]Using first {len(json_files)} files (--limit {limit}) for quick test.[/yellow]\n")
    ...
    # Local LLM on CPU can be ~1 min/request; LiteLLM (Qazcode key) is seconds
...
Examples:
  ...
  python main.py -e http://api.example.com/diagnose -d ./protocols -n team_alpha -p 10
    ...
    parser.add_argument(
        ...
        default=None,
        metavar="N",
        help="Use only first N files (quick test). Without limit, full dataset runs (slow with local LLM).",
    )
    ...
    # Check backend is reachable before running 221 requests
    async def check_backend():
        base = args.endpoint.replace("/diagnose", "")
        try:
    ...
    if not asyncio.run(check_backend()):
        console.print(
            "[red]Backend not reachable at[/red] [yellow]{}[/yellow]. "
            "Start it first:\n  [cyan]py -m uvicorn src.main:app --host 127.0.0.1 --port 8080[/cyan]\n"
            "Then run this script again.".format(args.endpoint.replace("/diagnose", "") + "/health")
        )
        return 1
    ...
    if results:
        ...
        # Always print main metrics to console
        console.print(
            "\n[bold green]Accuracy@1:[/bold green] {}%  [bold green]Recall@3:[/bold green] {}%".format(
                metrics["accuracy_at_1_percent"], metrics["recall_at_3_percent"]
            )
        )
    else:
        console.print("[yellow]No successful results — check errors above. Fix backend and re-run.[/yellow]")
